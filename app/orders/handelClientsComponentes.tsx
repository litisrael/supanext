...
const HandelClientsComponents = () => {
  ...
  >([]);
  const [dataRenderizar, setDataRenderizar] = useState<DataTransformadaItem[]>(
    []
  );
    ...
    const fetchDataCharat = async () => {
      try {
        ...
        if (
          ...
          const { data: dataVentasXdia, error: VentasXdia } =
            await supabase.rpc("obtener_cantidad_ventas_por_dia", {
              id_usuario: user.id,
  ...
  useEffect(() => {
    if (dataFetched) {
      ...
      const promedioCalculado = (totalCantidad / dataRenderizar.length).toFixed(
        2
      );
      const cardDataMap = [
    ...
    cantidadpPorSkuYFecha,
    checkedValues,
    dataFetched,
    // setRenderVentasXState
    ,
  ]);
  ...
  return (
    <>
      ...
      {/* <section className="flex flex-wrap justify-around gap-4 sm:gap-8 xl:gap-12"> */}
